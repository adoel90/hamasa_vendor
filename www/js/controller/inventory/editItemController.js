distributionApp.controller('editItemController', function($scope, $rootScope,
    $supplierService, $itemService, $timeout, $categoryService, $gradeService) {

    $rootScope.itemId = encodeURIComponent(routeParams.id);
    $scope.requiredData = {
        ig_id: '',
        id: '',
        serial: '',
    ...
    function updateItemMaster(data){
      $itemService.put.updateItem(data)
      .then(function(response) {
        $rootScope.triggerModal("Informasi barang telah berhasil diubah.", "Success", "success", "/inventory/itemmaster");
      })
    ...
    $scope.save = function() {
      ...
      setTimeout(()=>{
        updateItemMaster($scope.requiredData);
      }, 300);
    }
